"""T-3 집계 — 사전등록 c2b2a902… 문턱 + 안전장치 (E) ILS · (F) mix."""
import json, glob, collections, sys, os
from pathlib import Path
import numpy as np
ROOT = Path(__file__).resolve().parents[1]; sys.path.insert(0, str(ROOT))
K = json.load(open(ROOT / "eval/t3_key.json")); rows, todo = K["rows"], K["todo"]
key = {t["id"]: t for t in todo}; votes = collections.defaultdict(list)
for f in sorted(glob.glob(str(ROOT / "eval/t3/[ABC]_*.json"))):
...
json.dump(dict(rows=rows), open(ROOT / "eval/t3_graded.json", "w"), ensure_ascii=False)
...
L = json.load(open(ROOT / "eval/t3_lists.json"))
...
best = max(("w2", "w4"), key=lambda a: V[a][0]); d = V[best][0] - V["w0"][0]
print(f"\n최선 {best}: Δ합산 {d:+.4f} · Δmix {V[best][1]-V['w0'][1]:+.4f} · ΔILS {V[best][2]-V['w0'][2]:+.4f}")
if d >= 0.03:
    if V[best][2] >= V["w0"][2] + 0.03: print("판정: (E) ILS 안전장치 → 채택하지 않음")
    elif V[best][1] < V["w0"][1] - 0.05: print("판정: (F) mix 안전장치 → 채택하지 않음")
    else: print(f"판정: (A) 채택 → tag_w = {'0.2' if best=='w2' else '0.4'}")
elif d < 0.02: print("판정: (C) 0 유지")
else: print("판정: 보류 → 0 유지")
print("\n### 축별")
for ax in ("rule", "coh", "mix", "count"):
    print(f"  {ax:<6}" + "".join(f"  {a} {P([r for r in rows if r['arm']==a and prof[r['pid']]['axis']==ax]):.3f}" for a in ("w0","w2","w4")))
