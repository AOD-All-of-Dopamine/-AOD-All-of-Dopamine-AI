"""T-3 집계 — 사전등록 c01043a6… 문턱 + 안전장치 (E) ILS · (F) mix."""
import json, glob, collections, sys, os
from pathlib import Path
import numpy as np
ROOT = Path(__file__).resolve().parents[1]; sys.path.insert(0, str(ROOT))
K = json.load(open(ROOT / "eval/t5_key.json")); rows, todo = K["rows"], K["todo"]
key = {t["id"]: t for t in todo}; votes = collections.defaultdict(list)
for f in sorted(glob.glob(str(ROOT / "eval/t5/[ABC]_*.json"))):
...
json.dump(dict(rows=rows), open(ROOT / "eval/t5_graded.json", "w"), ensure_ascii=False)
...
L = json.load(open(ROOT / "eval/t5_lists.json"))
...
V = {}
print("\n### 팔별 (rep_v2 · pop 0 · tag_w 0.2 · top2_mean · 프로필 67)")
for a in ("h0", "h35", "h50"):
...
best = max(("h35", "h50"), key=lambda a: V[a][0]); d = V[best][0] - V["h0"][0]
print(f"\n최선 {best}: Δ합산 {d:+.4f} · Δmix {V[best][1]-V['h0'][1]:+.4f} · ΔILS {V[best][2]-V['h0'][2]:+.4f}")
if d >= 0.03:
    if V[best][2] >= V["h0"][2] + 0.03: print("판정: (E) ILS 안전장치 → 채택하지 않음")
    elif V[best][1] < V["h0"][1] - 0.05: print("판정: (F) mix 안전장치 → 채택하지 않음")
    else: print(f"판정: (A) 채택 → hub_lambda = {'0.35' if best=='h35' else '0.50'}")
elif d < 0.02: print("판정: (C) 0 유지")
else: print("판정: 보류 → 0 유지")
print("\n### 축별")
for ax in ("rule", "coh", "mix", "count"):
    print(f"  {ax:<6}" + "".join(f"  {a} {P([r for r in rows if r['arm']==a and prof[r['pid']]['axis']==ax]):.3f}" for a in ("h0","h35","h50")))
