"""T-5: hub_lambda 0 / 0.35 / 0.50 on rep_v2, pop 0, tag_w 0.2, top2_mean. h0 = T-3 w2 목록. seed 5. 은행 = T-1 ∪ T-4."""
...
FIX = dict(pop_boost=0.0, star_boost=0.0, tag_w=0.2, strategy="top2_mean")
lists = {f"h0|{k.split('|')[1]}": v for k, v in json.load(open(ROOT / "eval/t3_lists.json")).items() if k.startswith("w2|")}
chk = [int(x) for x in eng.recommend(P[0]["seeds"], k=50, hub_lambda=0.0, **FIX)["item_id"]]
assert chk == lists[f"h0|{P[0]["pid"]}"][:50], "기준선 재현 실패 — 설정 기본값이 새어 들어갔다"
for arm, hl in (("h35", 0.35), ("h50", 0.50)):
    for p in P:
        lists[f"{arm}|{p['pid']}"] = [int(x) for x in eng.recommend(p["seeds"], k=50, hub_lambda=hl, **FIX)["item_id"]]
json.dump(lists, open(ROOT / "eval/t5_lists.json", "w"))
E = eng.emb; row = eng.row
def ils(ids):
    M = E[[row[i] for i in ids[:50]]]; S = M @ M.T; iu = np.triu_indices(len(M), 1); return float(S[iu].mean())
for arm in ("h0", "h35", "h50"):
    v = [ils(lists[f"{arm}|{p['pid']}"]) for p in P]
    ov = np.mean([len(set(lists[f"{arm}|{p['pid']}"]) & set(lists[f"h0|{p["pid"]}"])) / 50 for p in P])
    same = sum(lists[f"{arm}|{p['pid']}"] == lists[f"h0|{p["pid"]}"] for p in P)
    import collections as _c; cnt = _c.Counter(i for p in P for i in lists[f"{arm}|{p['pid']}"][:50]); hubs = sum(1 for c in cnt.values() if c >= 20)
    print(f"  {arm}: 허브(≥20회) {hubs}편 · 최다 {max(cnt.values())}회 · ILS@50 {np.mean(v):.3f} · s_t2 와 겹침 {ov*100:.0f}% · 동일 목록 {same} · 고유 {len({i for p in P for i in lists[f'{arm}|{p['pid']}']}):,}")
...
bank = {}
for f in ("t1_graded.json", "t2_graded.json", "t3_graded.json", "t4_graded.json"):
    for r in json.load(open(ROOT / "eval" / f))["rows"]:
        if "maj" in r: bank[(r["pid"], r["item"])] = r["maj"]
rng = random.Random(5); rows = []
...
json.dump(dict(rows=rows, todo=todo), open(ROOT / "eval/t5_key.json", "w"), ensure_ascii=False)
nb = (len(todo) + 59) // 60
for b in range(nb):
    (SCR / f"t5_b{b}.json").write_text(json.dumps([{k: r[k] for k in ("id","seed","cand","meta")} for r in todo[b*60:(b+1)*60]], ensure_ascii=False))
