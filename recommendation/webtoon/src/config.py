...
PRODUCTION = {
    ...
    "tag_w": 0.2,              # T-3 +0.075 → **T-9 재현 +0.030 (B') 유지** · 부호 유지·크기 불확실(실효 +0.03~+0.05) · ILS 불변 · 피복 0.69→0.86. 0.4 는 −0.019. Steam D-49 와 달리 태그가 편집된 취향 라벨이라 통한다
    "min_favorite": None,      # 미측정
    "creator_w": 0.0,          # **T-8 재현 실패 → 0 확정, 축 종료**: T-7 +0.028 → T-8 −0.020 (합산 +0.004). mix +0.092 → −0.067. 같은 쌍 재채점 적합 일치 0.827
    "tag_drop_genre": False,   # T-10 판정 전 현행. True = 태그 피복에서 장르명 태그 제외 (태그 1개=장르명인 작품 30% → 피복 포화)
    "dislike_w": 0.0,          # **T-11 (C) 기각 → 0 유지**: 닮은 작품 감소 w1/2/3 0.62/0.57/0.54배(문턱 0.50) · 품질 Δ −0.012~−0.019 · 무관 가드 0.996. 원인은 floor(표적 42% 가 아래). 0 이면 싫어요 작품·그 시리즈 제외만 한다
    "dislike_floor": 0.61,     # T-11 단계 1: 무관 싫어요 유사도 p99(0.600) 를 0.01 단위 올림 (src/t11_floor.py)
}
POSTPROCESS = {
