...
#
# 2026-08-15 — 이 게이트를 **시드 응집도**로 바꾸려던 시도는 기각됐다.
# 논리는 이랬다: 합의 태그는 시드들이 실제로 한 축을 공유할 때만 의미가 있으니,
# "인기"가 아니라 "응집"으로 켜는 것이 맞다. 응집도는 라벨 없이 잴 수 있다 —
# 시드 임베딩들의 평균 쌍별 유사도를 무작위 쌍 분포(4,000개 표본: 중앙 0.460 ·
# p90 0.565 · p99 0.657)와 비교하면 된다. 임계값은 판정을 보기 **전에** p90 으로 고정했다.
#
# 결과: **선언된 coherent/mixed 를 전혀 못 가른다.**
#   mix2_crpg_sandbox (mixed 선언)   0.6354  ← 응집 판정. coh_fps 와 같은 값
#   coh_arpg  0.5322 · coh_cozy 0.5292 · niche_tactics 0.5198  (전부 coherent 선언인데 혼합 판정)
# 게다가 응집도와 P@100 사이에 관계가 없다: coh_survival_craft 0.72 → 0.93 인데
# coh_mmo 0.695 → 0.57 이다. 임계값을 옮겨도 순서가 안 바뀌므로 다른 값을 재볼 필요가 없다.
...
def postprocess(
    ...
    ────────────────────────────────────────────────────────────────────────
    2026-08-14 — 합의 태그로 **순위를 매기는** 시도 전부 기각. 다시 하지 말 것.

    동기는 실재했다. 43프로필로 넓히자 실패한 8축의 합의 태그가 정확히 "잃어버린
    축"을 가리키고 있었다(two_rhythm_arcade → {Rhythm, Music}, coh_mmo → {MMORPG},
    coh_sports → {Sports}, coh_jrpg → {JRPG}). 전부 인기 조건(10만+)에서 탈락해
    한 번도 안 걸렸고, 정작 조건을 통과한 twenty_broad 의 합의는 43개라 무의미했다.

    시도 1 — 하드컷을 소프트 분할(OR)로. **무효.** 합의 집합에 흔한 태그가 섞여
      있어 후보 대부분이 하나는 갖는다. 43프로필 재생성에서 0히트 부적합 후보가
      coh_mmo 43개 중 0개, 바뀐 칸 coh_mmo 0 · coh_sports 0 · coh_jrpg 0.

    시도 2 — 맞힌 **개수**로 사전식 정렬. 측정됨(243쌍 추가 판정, k=30):
        신규 8축  0.6375 → 0.7208   (two_rhythm_arcade 0.43 → 0.77)
        기존 35축 0.9514 → 0.8990   (mix2_crpg_sandbox 0.90 → 0.67)
        전체      0.8930 → 0.8659   미달 8 → 11개    ← 순손실, 기각
      * 이때 `interleave_by_seed` 가 버킷 안을 final_score 로 재정렬해 순서를 통째로
        버리는 버그를 함께 발견했다(합의 히트 평균 coh_mmo 4.20 → 4.20 무변화).
        아래 시도들은 그 버그를 고친 상태에서 측정한 것이다.

    시도 3 — max_df 0.15 → 0.05 + 메타 태그 제외 + IDF 가중. **역효과, 기각.**
      빈도를 조이면 **장르 태그가 먼저 잘리고 맥락 태그가 남는다**:
        coh_fps         FPS 5.8% · Shooter 10.2% 탈락 → {eSports 0.8%, Competitive 0.9%}
                        → EA SPORTS FC · NBA 2K26 · Dota 2 가 3~17위로 진입
        niche_roguelite Roguelike 7.7% · Roguelite 7.4% 탈락 → {Souls-like, Isometric}
        coh_indie_platformer  Platformer 9.5% 탈락 → {Souls-like, Metroidvania}

    **결론: 빈도로는 "정의적"과 "부수적"을 가를 수 없다.** 스팀의 드문 태그는
    오히려 기분·맥락·품질 서술(Beautiful 0.3%, eSports 0.8%, Competitive 0.9%)에
    몰려 있고, 장르를 말하는 태그는 흔한 쪽(FPS, Shooter, Platformer, Roguelike)에
    있다. 개수·IDF·태그 위치가중 셋 다 같은 프로필에서 같은 방향으로 실패했다
    (적합/부적합 분리도가 음수: niche_cozy_casual −0.18 · mix2_crpg_sandbox −0.14 ·
    longtail_metroidvania −0.15) — 그리고 그 셋이 정확히 회귀 폭 1~3위였다.

    진짜 규칙은 이것이다: **합의 순위는 랭커가 축을 놓친 프로필에서만 이득이고,
    이미 축을 맞힌 프로필에서는 순수한 잡음이라 손해만 난다.** 이걸 라벨 없이
    가르는 방법을 아직 못 찾았다. 판정 결과를 보고 임계값을 고르면 인기 조건이
    저질렀던 것과 똑같은 과적합이 되므로 그렇게 하지 않았다.

    다음에 볼 곳은 후처리가 아니라 **임베딩**이다. two_rhythm_arcade 가 max 집계에서도
    top2_mean 과 똑같이 실패한다는 것(둘 다 P@100 0.28)이 그 증거다 — NecroDancer 의
    임베딩 최근접 이웃 자체가 리듬게임이 아니라 코옵 던전 로그라이크다. semantic_text
    에서 태그 15개가 설명문에 묻히는 구조를 의심할 것.
    ────────────────────────────────────────────────────────────────────────
    """
    df = ranked
    if drop_dead_mp:
