...
- 상태: **v2.8** (2026-09-26) — 변경 이력
  - **v2.8**: **추천 탭(`/for-you`)을 없애고 홈 한 줄로 합쳤다**(프론트 #55 · 백엔드 #127, 2026-09-26 운영 배포). 화면 기준은 이제 프론트 `docs/superpowers/specs/2026-09-26-home-rec-only-design.md` 다.
    §2(화면)는 추천 탭 기준 기록으로 남기고, 바뀐 곳에 표시만 달았다 — §2-1 진입 · §2-6 묶음 30개 + 새 추천 받기 · §2-8 U1~U5 닫힘 · §2-9 폐기 ·
    §4-1 `size` 1~30 · §5-4 노출 중복 서술 · §7-1 · §12-5·10·11 결정 · 부록 C · 부록 D B9. 로그 · API · 백엔드 · 추천 서비스 절은 그대로 유효하다
...
> **2026-09-26 — 추천 탭은 홈으로 합쳤다.** `/for-you` 는 `/home?rec=<칩>` 으로 넘어가고, 추천은 홈의 한 줄(분야 칩 · 30개 가로 줄 · 순환 화살표 ·
> "새 추천 받기")에서만 보인다. 지금 화면의 기준은 프론트 `docs/superpowers/specs/2026-09-26-home-rec-only-design.md` 다. 아래 §2-1~§2-9 는 추천 탭을 설계 · 배포했던 기록이며,
> 바뀐 곳은 절마다 표시했다. **§2-3 이유 · §2-4 피드백 원칙 · §2-5 콜드스타트 · §2-7 대체 규칙은 홈에서도 그대로 쓴다.**

### 2-1. 진입
> **2026-09-26 변경** — 진입은 **홈 한 곳**. 상단 메뉴 "추천" · 모바일 홈 세그먼트 · `/for-you` 페이지 모두 없앴다(옛 주소는 홈으로 넘긴다). 칩은 URL `?rec=`.

- (기록) 경로 `/for-you`. **메뉴가 이미 6개**라 7번째로 넣지 않는다:
...
### 2-4. 피드백 조작
**피드백은 카드 표면에 둔다 (2026-09-21 결정).** 메뉴 안에 숨기면 아무도 누르지 않고, 신호가 안 쌓이면 추천은 영원히 그대로다.
넷플릭스는 별점을 엄지로 바꾼 시험에서 평가 활동이 200% 늘었다.

| 조작 | 위치 | 즉시 화면 | 서버 |
|---|---|---|---|
| 좋아요 👍 | **카드 표면** | 채워진 버튼 · **카드는 그대로 둔다** | `PUT /api/works/{id}/reaction {state: LIKE}` |
| 싫어요 👎 | **카드 표면** | **그 자리에서 가려짐**(자리 유지) + "덜 보여드릴게요 · 되돌리기" | `PUT … {state: DISLIKE}` → 응답의 `previousState` 보관 |
| 되돌리기 | 가려진 카드 자리 | 카드 복원 | `PUT … {state: previousState}` — 좋아요였던 작품은 좋아요로 돌아간다 |
| 이번엔 넘길게요 | ⋯ 메뉴 | 가려짐 + 되돌리기 | `PUT /api/recommendations/not-interested/{id}` · 되돌리기 `DELETE` |
| 북마크 | ⋯ 메뉴 | 표시 | 기존 토글 API |

- **표면에는 👍/👎 둘만.** 싫어요와 "이번엔 넘길게요"를 나란히 두면 차이를 구분하지 못한다 — 후자는 ⋯ 메뉴에 남긴다.
...
**가려짐 · 되돌리기**
- 목록에서 빼지 않고 **자리를 유지한 채 흐리게** 만든다 — 카드를 빼면 아래가 밀려 올라와 방금 누른 것이 시야에서 사라지고 되돌리기가 어렵다.
- 되돌리기는 **토스트 5초가 아니라 그 자리에 남긴다.** 정리 시점: 페이지를 떠나거나 "더 보기"를 눌러 다음 묶음을 받을 때.
- **낙관적 반영 + 실패 시 복구**: 화면을 먼저 바꾸고, API 가 실패하면 원래대로 되돌리고 오류를 알린다.
- 가려진 카드는 노출 추적을 멈춘다(§5-4) — 가려진 상태는 "보인 노출"이 아니다.
...
- 피드백 직후 화면 반응(가려짐·되돌리기)은 세 회사 모두 공개 문서에 없다 — **우리 설계 선택**이다.
- **효과 확인**: 넣은 뒤 싫어요·관심 없음 비율이 **오르면** 신호가 쌓이기 시작한 것이고, 그 뒤 시간이 지나며 **내려가면** 추천이 실제로 좋아진 것이다(당근: 숨기기 고도화 후 숨기기 사용 −40%).
...
> **2026-09-26 변경 (홈 한 줄)** — "더 보기"로 쌓지 않고 **한 묶음 30개를 가로 한 줄**로 보여 준다.
> - **"새 추천 받기"** = 같은 체인의 **다음 쪽**(서버가 본 작품을 빼고, 요청마다 시드를 새로 읽어 그사이 👍/👎 를 반영). 화면에는 **마지막 묶음만** 보인다("새로 고른 추천 · N번째").
>   아래 "새로고침 버튼은 두지 않는다"는 **같은 목록을 다시 받는** 버튼에 대한 판단이고, 이 버튼은 다음 묶음을 받으므로 그 우려에 해당하지 않는다.
> - **처음부터 다시 받기** = 새 체인(아래 "새로 보기"와 같다). 이어 받은 쪽이 대체 응답이면 버리고 보던 묶음을 유지한다.
> - 체인은 **캐시가 없어도 이어 받는다**(새로고침 → 다음 묶음). 더 받을 게 없는 체인(`hasMore:false`)은 기억하지 않는다.
> - 가로 위치는 칩 · 묶음별로 복원한다. 상세 기준은 프론트 설계 문서.
...
> **✅ 닫힘 (2026-09-26)** — U1~U4 는 추천 탭과 함께 없어졌다. 홈 카드는 처음부터 **표면 👍/👎(엄지) · 자리 유지 가려짐 · 라벨 "좋아요"/"관심 없어요"** 로 만들었다(홈 설계 2026-09-25).
> **U5 는 "30개 한 묶음 + 새 추천 받기"로 결정**했다 — 아래 권고(10개 · "더 보기" 유지)를 **뒤집은 것**이다. 추천 탭이 없어져 이어 보기를 "더 보기" 대신 새 묶음 버튼이 맡고,
> 가로 한 줄이라 30개도 한 화면 높이다.
...
> **폐기 (2026-09-26)** — 추천 탭을 없애 실행하지 않았다. 계획한 조각(`RecThumbs` · `HiddenCardSlot` · `slotVisible`)은 홈 추천이 쓴다.
...
**추천 조회**
```
GET /api/recommendations?tab=all|movie|tv|game|webtoon|webnovel&chainId=&size=20&surface=home_rec
...
```
- `size` 는 **1~30**(기본 20). 홈은 한 묶음 **30**을 보낸다(백엔드 #127 — 엔진 k 는 늘지 않는다: `buffer = 50 - size`).
- `surface` 는 `rec_request.surface` 에 적힌다. 허용 `{rec_tab, home_rec}`, 없거나 모르는 값은 `rec_tab`. 추천 탭이 없어진 뒤로 모든 요청이 `home_rec` 이다.
...
- **50%·1초를 넘는 순간 1회 전송**(모바일 탭 종료 유실 대비) + 화면을 벗어나거나 페이지를 떠날 때 **최종값 1회**(`final=true`). ~~체인 안 카드당 최대 2건.~~
  **보장되지 않는다(2026-09-26 확인)** — 카드가 다시 마운트되면(칩 왕복 · 상세 뒤로가기 · 되돌리기) 새 계측기가 같은 `impression_id` 로 또 보낸다. **분석은 `DISTINCT impression_id` 로 센다.** 가로 줄 순환은 스크롤만 옮기므로(복제 없음) 중복을 늘리지 않는다.
...
> **2026-09-26 — 추천 탭 제거, 홈으로 통합.** `/for-you` · `RecTabPage` · `RecCard`(`RecCardTile`) · ⋯ 메뉴는 삭제됐다. 지금은 `HomeRecRail` · `HomeRecCard` · `HomeRecEndCard` · `HiddenCardSlot` · `RecThumbs`(모두 ✅).
...
5. ~~모바일 진입 = 홈 세그먼트 (메뉴 7개 회피)~~ → **결정(2026-09-26): 진입은 홈 한 곳**(추천 탭 제거)
6. 코퍼스 갱신 주 1회
7. 담당자
8. 추천 호스트 사양 (초안: 16GB · 4코어 이상)
9. 엔진 하나가 준비되지 않아도 라우터를 띄울지 (초안: 전부 준비 후 시작)
10. ~~한 배치 10개 vs 20개 · "새로고침" 도입 여부~~ → **결정(2026-09-26): 한 묶음 30개 가로 줄 + "새 추천 받기"(다음 쪽) · "처음부터 다시 받기"(새 체인)** (§2-6 · §2-8)
11. ~~좋아요 아이콘 — 엄지 vs 하트~~ → **결정: 엄지**(홈 카드 `RecThumbs`, 라벨 "좋아요" 고정 + `aria-pressed`)
...
- 상세 페이지 "비슷한 작품" · ~~홈 추천 섹션 · 넷플릭스식 가로 행~~(✅ 2026-09-26 홈 한 줄)
...
- [x] 4. `/for-you` · 칩 · 반응형 그리드 · 더 보기 · 빈 상태·대체 (§2-1·§2-2·§2-7) — **2026-09-26 홈 한 줄로 대체**(칩 · 30개 · 순환 · 새 추천 받기, 프론트 #55)
- [x] 5. ~~`RecCard` — 표면 👎·가려짐 자리·라벨·아이콘~~ → 홈 카드(`HomeRecCard` · `RecThumbs` · `HiddenCardSlot`)로 해소, 추천 탭 카드는 삭제
...
- [x] 8. 배치 크기·새로고침 결정 반영 — 30개 한 묶음 + 새 추천 받기 (§12-10)
...
#### A5. 전체 탭 M6 혼합 k 〔추〕 — ✅ 해결 (2026-09-26 `2c0b06b`: 앞 k 개 = M6@k, 버퍼는 뒤에)
...
| B9 | ~~피드백 UI 가 설계와 어긋난다(👎가 ⋯ 메뉴 안 · 가려짐이 토스트 · 라벨·아이콘 불일치)~~ | ✅ **해소(2026-09-26)** — 추천 탭 제거, 홈 카드가 표면 👍/👎 · 자리 유지 가려짐 · 엄지로 되어 있다(프론트 #51 · #55) | 프 |
