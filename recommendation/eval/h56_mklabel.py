"""h56 fun_tag 라벨 입력 — h55 고정 풀. 등록 md5 386765b2962e63ed3fcfc73c362f9328."""
...
for ss,blk in P.items():
    for rd,rn in blk['profile']:
        if (rd,rn) not in tags: prof_missing.append((rd,rn))
    for arm in ('D','C'):
        for dom in DOMS:
            for c in blk[arm][dom]:
                slots+=1
                if (dom,c['n']) not in tags: need[(dom,c['n'])]=TX[dom][c['n']]
print(f"고정 풀 슬롯 {slots} · 프로필 라벨 누락 {len(set(prof_missing))} · 신규 라벨 필요 {len(need)}건")
if prof_missing: raise SystemExit("[중단] 프로필 항목 라벨 누락")
...
for pos,i in enumerate(pm): items[i]['id']=f"V{pos:03d}"
json.dump([{k:i[k] for k in ('id','dom','name')} for i in items],
          open(f"{SP}/h56_key.json","w"), ensure_ascii=False, indent=1)
order=list(range(len(items))); rng.shuffle(order)
bl=[f"## {items[i]['id']}  ({items[i]['dom']}) {items[i]['name']}\n{items[i]['text']}\n" for i in order]
for k in range(0, len(bl), 50):
    body="\n".join(bl[k:k+50])
    open(f"{SP}/vl{k//50+1}.txt","w").write(body)
    print(f"  vl{k//50+1}.txt {len(bl[k:k+50])}건 {len(body):,}자")
