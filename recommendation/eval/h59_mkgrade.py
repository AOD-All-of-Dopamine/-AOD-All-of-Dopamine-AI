"""h59 채점표 — 규칙 A·B 각각 자기 프로필 기준. 기존 등급 재사용, 신규만."""
...
for pos,i in enumerate(pm): need[i]['pid']=f"R{pos:03d}"
json.dump([{k:p[k] for k in ('pid','rdom','rname','dom','cand')} for p in need],
          open(f"{SP}/h59_grade_key.json","w"), ensure_ascii=False, indent=1)
...
for k in range(0, len(bl), 30):
    body="\n".join(bl[k:k+30])
    open(f"{SP}/rg{k//30+1}.txt","w").write(body)
    print(f"  rg{k//30+1}.txt {len(bl[k:k+30])}쌍 {len(body):,}자")
