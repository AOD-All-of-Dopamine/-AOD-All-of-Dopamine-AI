...
def compute_diagnostics(
    ...
    row_of = dict(zip(idx["item_id"], idx["embedding_row"]))
...
def compute_ranking_diagnostics(
    ...
    dataset = pd.read_parquet(artifact_dir(artifacts) / "dataset.parquet").set_index("item_id")
    ...
    for _, p in profiles.iterrows():
        r = run_multi(
            list(p["liked_ids"]), strategies=["max"], top_n=max(ks),
        ...
        for k in ks:
            ...
            counts = pd.Series(
                [g for a in t["item_id"] for g in dataset.loc[a, "genres"]]
            ).value_counts()
            probs = counts / counts.sum()
            acc[k]["genres"].append(float(np.exp(-(probs * np.log(probs)).sum())))
            acc[k]["series"].append(
                len({series_key(dataset.loc[a, "name"]) for a in t["item_id"]}) / len(t)
            )
        # 페이지 간 시드 구성 변화
...
def compute_loo(pop_boost: float = 0.03) -> dict:
    ...
    profiles = pd.read_parquet(PROJECT_ROOT / "artifacts" / "p1" / "profiles.parquet")
    loo = evaluate_leave_one_out(profiles, pop_boost=pop_boost)
