...
QUERY_PROMPT = (
    "Instruct: Given a Korean web novel, retrieve other Korean web novels "
    "that a reader who likes this novel would likely find relevant.\n"
    "Query: "
)

EXPERIMENT_ID = "webnovel_wn_v1_qwen"
...
def select_targets(df: pd.DataFrame, out: Path) -> tuple[pd.DataFrame, pd.DataFrame | None]:
    ...
    수만 건을 한 번에 임베딩하면 오래 걸린다. 그런데 품질 하한을 통과해 실제로 추천에
    나가는 것은 관심 수가 붙은 작품뿐이다(`has_interest`). 그래서 **가치 있는 것부터
    임베딩하고 나머지는 나중에 이어붙이는** 두 단계로 나눈다.

        1단계: --only-with-interest  관심 수가 있는 작품만
        2단계: --append              나머지를 이어붙임 (필요해지면)

    `--append` 는 기존 corpus_index 에 있는 item_id 를 건너뛴다. 부분 커버리지는
    ...
    if "--only-with-interest" in sys.argv:
        before = len(df)
        if "has_interest" not in df.columns:
            raise SystemExit("dataset 에 has_interest 컬럼이 없습니다.")
        df = df[df["has_interest"].fillna(False).astype(bool)]
        print(f"--only-with-interest: {before:,} → {len(df):,}건 "
              f"({len(df) / before * 100:.0f}%, 관심 수가 붙은 작품만)")
...
def main():
    ...
    # 앵커는 Steam 의 S1(아이템-투-아이템) 평가 트랙 산출물이다. 이 파이프라인은 P1
    # (멀티시드 개인화)만 쓰므로 없는 것이 정상이고, 있으면 함께 임베딩한다.
    anchor_path = src / "anchors_40.parquet"
    anchors = pd.read_parquet(anchor_path) if anchor_path.exists() else None
    if out != src:
    ...
    corpus_index = df[["item_id", "name"]].reset_index(drop=True)
