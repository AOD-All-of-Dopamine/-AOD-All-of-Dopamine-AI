"""W-3: rating_boost 0 / 0.10 / 0.20 on wn_v6, pop 0, top2_mean. r0 = W-1 p0 목록. seed 9. 은행 = W-1 ∪ W-2."""
...
lists = {}
for arm, rb in (("r0", 0.0), ("r10", 0.10), ("r20", 0.20)):
    for _, r in P.iterrows():
        seeds = [int(s) for s in r["seed_ids"]]
        lists[f"{arm}|{r.profile_id}"] = [int(x) for x in eng.recommend(seeds, k=50, pop_boost=0.0, rating_boost=rb)["item_id"]]
json.dump(lists, open(ROOT / "eval/w3_lists.json", "w"))
...
for arm in ("r0", "r10", "r20"):
    v = [ils(lists[f"{arm}|{r.profile_id}"]) for _, r in P.iterrows()]
    ov = np.mean([len(set(lists[f"{arm}|{r.profile_id}"]) & set(lists[f"r0|{r.profile_id}"])) / 50 for _, r in P.iterrows()])
    ...
    print(f"  {arm}: ILS@50 {np.mean(v):.3f} · r0 와 겹침 {ov*100:.0f}% · 관심수 중앙 {fav:,.0f} · 고유 {uniq:,}")
meta = {}
...
BANK = {(r["pid"], r["item"]): r["maj"] for r in json.load(open(ROOT / "eval/w1_graded.json"))["rows"] if "maj" in r} | {(r["pid"], r["item"]): r["maj"] for r in json.load(open(ROOT / "eval/w2_graded.json"))["rows"] if "maj" in r}
need, seen = [], set()
for r in rows:
    k = (r["pid"], r["item"])
...
json.dump(dict(rows=rows, todo=todo), open(ROOT / "eval/w3_key.json", "w"), ensure_ascii=False)
nb = (len(todo) + 59) // 60
for b in range(nb):
    (SCR / f"w3_b{b}.json").write_text(json.dumps([{k: r[k] for k in ("id", "seed", "cand", "meta")} for r in todo[b*60:(b+1)*60]], ensure_ascii=False))
