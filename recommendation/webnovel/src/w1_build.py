"""W-1: pop_boost 0 / 0.03 / 0.10 재판정 on wn_v6 (교정 프로필). seed 7. 은행 재사용 없음(1인 채점 오염 방지)."""
...
lists = {}
for arm, pb in (("p0", 0.0), ("p03", 0.03), ("p10", 0.10)):
    for _, r in P.iterrows():
        seeds = [int(s) for s in r["seed_ids"]]
        lists[f"{arm}|{r.profile_id}"] = [int(x) for x in eng.recommend(seeds, k=50, pop_boost=pb)["item_id"]]
json.dump(lists, open(ROOT / "eval/w1_lists.json", "w"))
...
for arm in ("p0", "p03", "p10"):
    ...
    ov = np.mean([len(set(lists[f"{arm}|{r.profile_id}"]) & set(lists[f"p0|{r.profile_id}"])) / 50 for _, r in P.iterrows()])
    ...
    print(f"  {arm}: ILS@50 {np.mean(v):.3f} · p0 와 겹침 {ov*100:.0f}% · 관심수 중앙 {fav:,.0f} · 고유 {uniq:,}")
meta = {}
...
json.dump(dict(rows=rows, todo=todo), open(ROOT / "eval/w1_key.json", "w"), ensure_ascii=False)
nb = (len(todo) + 59) // 60
for b in range(nb):
    (SCR / f"w1_b{b}.json").write_text(json.dumps([{k: r[k] for k in ("id", "seed", "cand", "meta")} for r in todo[b*60:(b+1)*60]], ensure_ascii=False))
by = collections.Counter(r["arm"] for r in rows)
print("슬롯:", dict(by)); print(f"신규 채점 {len(todo)} · 배치 {nb}")
