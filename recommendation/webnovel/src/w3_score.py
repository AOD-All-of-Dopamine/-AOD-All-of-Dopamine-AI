"""W-1 집계 — 사전등록 1f6621ae… 문턱 + 안전장치 (E) ILS · (F) 저관심."""
import json, glob, collections, sys, os
from pathlib import Path
import numpy as np, pandas as pd
ROOT = Path(__file__).resolve().parents[1]; sys.path.insert(0, str(ROOT))
K = json.load(open(ROOT / "eval/w3_key.json")); rows, todo = K["rows"], K["todo"]
key = {t["id"]: t for t in todo}; votes = collections.defaultdict(list)
for f in sorted(glob.glob(str(ROOT / "eval/w3/[ABC]_*.json"))):
...
json.dump(dict(rows=rows), open(ROOT / "eval/w3_graded.json", "w"), ensure_ascii=False)
...
eng = Engine(); L = json.load(open(ROOT / "eval/w3_lists.json"))
...
best = max(("r10", "r20"), key=lambda a: V[a][0]); d = V[best][0] - V["r0"][0]
print(f"\n최선 {best}: Δ합산 {d:+.4f} · Δ저관심 {V[best][1]-V['r0'][1]:+.4f} · ΔILS {V[best][2]-V['r0'][2]:+.4f}")
if d >= 0.03:
    if V[best][2] >= V["r0"][2] + 0.03: print("판정: (E) ILS 안전장치 → 채택하지 않음, pop_boost=0.0 으로 되돌린다")
    elif V[best][1] < V["r0"][1] - 0.05: print("판정: (F) 저관심 안전장치 → 채택하지 않음, pop_boost=0.0 으로 되돌린다")
    else: print(f"판정: (A) 채택 → rating_boost = {'0.10' if best=='r10' else '0.20'}")
elif d < 0.02: print("판정: (C) 0 유지 — D-62 재확인")
