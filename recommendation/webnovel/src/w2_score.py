"""W-1 집계 — 사전등록 5ad382c6… 문턱 + 안전장치 (E) ILS · (F) 저관심."""
import json, glob, collections, sys, os
from pathlib import Path
import numpy as np, pandas as pd
ROOT = Path(__file__).resolve().parents[1]; sys.path.insert(0, str(ROOT))
K = json.load(open(ROOT / "eval/w2_key.json")); rows, todo = K["rows"], K["todo"]
key = {t["id"]: t for t in todo}; votes = collections.defaultdict(list)
for f in sorted(glob.glob(str(ROOT / "eval/w2/[ABC]_*.json"))):
...
json.dump(dict(rows=rows), open(ROOT / "eval/w2_graded.json", "w"), ensure_ascii=False)
...
eng = Engine(); L = json.load(open(ROOT / "eval/w2_lists.json"))
...
print(f"\n최선 {best}: Δ합산 {d:+.4f} · Δ저관심 {V[best][1]-V['s_t2'][1]:+.4f} · ΔILS {V[best][2]-V['s_t2'][2]:+.4f}")
if d >= 0.03:
    if V[best][2] >= V["s_t2"][2] + 0.03: print("판정: (E) ILS 안전장치 → 채택하지 않음, pop_boost=0.0 으로 되돌린다")
    elif V[best][1] < V["s_t2"][1] - 0.05: print("판정: (F) 저관심 안전장치 → 채택하지 않음, pop_boost=0.0 으로 되돌린다")
